...
from utility.create_mixture_helper_ss_only import read_sources, get_list_from_csv, transform_sources, loudness_normalize, mix_sources, mix_noise, check_for_clipping
...
parser.add_argument('--mixture_type', type=str, default="['clean_sources', 'mix_sources', 'reverb_noisy']",
...
def process_utterances(md_df, reverb_df, n_src, data_dir, noise_dir, data_outdir, mode, mixture_type, dataset):
    ...
    for i in tqdm(range(len(md_df))):
        ...
        if 'clean_2sources_anechoic' in outpath_dict.keys():
            for i in range(n_src):
                sf.write(os.path.join(outpath_dict['clean_2sources_anechoic'], f's{i+1}', mixture_id + '.wav'), np.array(anechoic[0, i, :len(mixture)]) * s1_spatial_scaling, FREQUENCY)
