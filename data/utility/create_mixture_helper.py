...
def transform_sources(sources_list, gain_list, padding_list, n_src, mixture_length):
    ...
    for i in range(n_src):
        left_padding = int(padding_list[i])
        right_padding = int(mixture_length - left_padding - len(sources_list[i]))
        assert right_padding >= 0, f"right_padding = {right_padding} is smaller than 0"
        sources_list_norm[i] = np.pad(sources_list_norm[i], (left_padding, right_padding), mode='constant')
